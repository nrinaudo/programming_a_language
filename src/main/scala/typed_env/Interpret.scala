...
def runAdd[Γ <: Tuple](lhs: TypedExpr[Type.Num, Γ], rhs: TypedExpr[Type.Num, Γ], e: Env[Γ]) =
  interpret(lhs, e) + interpret(rhs, e)

def runGt[Γ <: Tuple](lhs: TypedExpr[Type.Num, Γ], rhs: TypedExpr[Type.Num, Γ], e: Env[Γ]) =
  interpret(lhs, e) > interpret(rhs, e)

def runCond[X <: Type, Γ <: Tuple](
...
def runLet[X <: Type, Y <: Type, Γ <: Tuple](
    value: TypedExpr[X, Γ],
...
def runLetRec[X <: Type, Y <: Type, Γ <: Tuple](
...
def runRef[X <: Type, Γ <: NonEmptyTuple](loc: Elem[X, Γ], e: Env[Γ]): Value[X] =
  (e, loc) match
    case (Env.Cons(found, _), Elem.Here())     => found.nn
    case (Env.Cons(_, tail), Elem.There(rest)) => runRef(rest, tail)

def runFun[X <: Type, Y <: Type, Γ <: Tuple](body: TypedExpr[Y, X *: Γ], e: Env[Γ]): Value[X -> Y] =
  x => interpret(body, e.push(x))

def runApply[X <: Type, Y <: Type, Γ <: Tuple](fun: TypedExpr[X -> Y, Γ], arg: TypedExpr[X, Γ], e: Env[Γ]) =
